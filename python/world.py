...
class Terrain:
	def __init__(self):
		...
		self.worldsize_y = 200
		self.worldsize_x = 200
		...
		self.height = 800
		self.ligne = []
		self.map = [self.ligne] * self.worldsize_x

	def generate(self):
		pass

	def Render_map(self, offset_x, offset_y):
		pass

	def GenerateNoiseTexture(self):
		pic = []
		for i in range(self.worldsize_x):
			row = []
			for j in range(self.worldsize_y):
	...
	def savemap(self):
		save = open("otherfile/map.txt", 'w')
		save.writelines("")
		save.close()

		save = open("otherfile/map.txt", "a")
		for element in self.map:
			save.writelines(str(element)+"\n")
		save.close()